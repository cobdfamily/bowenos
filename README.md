...
- ext4 `/persist` on:
  ...
  - `bootbById` when `diskMode = "persistent"`
...
- `diskMode` — `mirror`, `single`, or `persistent`
...
For `diskMode = "mirror"` or `diskMode = "persistent"`, setup and partitioning require at least two distinct disks.
